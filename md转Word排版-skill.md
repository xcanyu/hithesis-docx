...
> **前置依赖**：此 skill 基于 [hithesis-docx](/) 项目，需先安装该项目（含 `.venv`）才能使用。
...
2. Search `~/Desktop/hithesis-docx`, `~/Documents/hithesis-docx`, or similar

If found, use that path. If NOT found, ask the user:

> "需要 hithesis-docx 项目才能转换。请提供项目路径，或者从 https://github.com 克隆一份。"

Do NOT proceed with conversion until the project path is confirmed.

The project uses a `.venv` (Python 3.12, dependencies installed). Activate with:
```
...
```powershell
# Navigate to project first (expand ~ to $env:USERPROFILE)
Set-Location "$env:USERPROFILE\Desktop\hithesis-docx"
.venv\Scripts\activate
...
If available, check `~/Desktop/hithesis-exam/convert_exam.py` for a complete example of complex markdown parsing with options, answers, and adaptive heading levels.
